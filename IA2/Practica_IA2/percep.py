...
def change_color():
    ...
    if good_or_bad:
        red_green_button.config(background="#2CA711")
    else:
        red_green_button.config( background="#FF0000")
...
#button
start_button = Button(mainwindow, text="Perceptron AND", command=lambda:threading.Thread(target=print_line).start())
start_button.place(x=600, y=230)

start_button = Button(mainwindow, text="Limpiar", command=clean_screen)
start_button.place(x=600, y=260) 

red_green_button = Button(mainwindow, background="#2CA711", width=10, command=change_color)
